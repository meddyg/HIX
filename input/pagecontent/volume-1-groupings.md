...
**Document Registry.** Es el Document Registry de MHDS. MHDS fija sus agrupaciones[^mhds-groupings] y HIX le añade una, porque el miembro publica con su identidad local y la comunidad la traduce por él.
...
- Un [Patient Identifier Cross-reference Consumer](appendix-glossary.html#patient-identifier-cross-reference-consumer) de PIXm, que HIX añade, con el que comprueba al indexar que la persona existe y está activa en la comunidad, como MHDS exige al validar una publicación[^mhds-subject], y con el que resuelve la identidad local de un puntero a su identidad maestra cuando la publicación le llega sin resolver.
